...
func cliBackend(runner *runtest.Recorder, prompter *countingPrompter) *KeePassXCCLIBackend {
...
func TestKeePassXCCLIWithNoDatabaseSaysSo(t *testing.T) {
	b := &KeePassXCCLIBackend{Runner: &runtest.Recorder{}, Prompter: &countingPrompter{password: "p"}}
	_, _, err := b.Lookup(t.Context(), defaultServicePrefix+"-id_ed25519")
