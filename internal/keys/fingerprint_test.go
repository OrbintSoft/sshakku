...
func TestFileFingerprint(t *testing.T) {
	...
	t.Run("unreadable key yields empty fingerprint, no error", func(t *testing.T) {
		r := runtest.NewRunner().On("ssh-keygen", runtest.Stdout("", 1))
...
func TestFingerprintLookupsAreBounded(t *testing.T) {
	t.Run("reading a key file", func(t *testing.T) {
		r := runtest.NewRunner().On("ssh-keygen", runtest.Stdout("256 SHA256:abc user@host (ED25519)\n", 0))
...
func TestRunnerFingerprinter(t *testing.T) {
	r := runtest.NewRunner().
		On("ssh-keygen", runtest.Stdout("256 SHA256:abc123 user@host (ED25519)\n", 0)).
		On("ssh-add", runtest.Stdout("256 SHA256:abc123 one (ED25519)\n", 0))
