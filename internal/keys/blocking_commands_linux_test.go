...
func platformBlockingCases(ctx context.Context, brief time.Duration) []blockingCase {
	return []blockingCase{
		{"GUI detection (xset)", func() {
			HasGraphicalSession(ctx, GUIEnv{Display: ":0"}, run.ExecRunner{})
		}},
		{"graphical passphrase prompt (kdialog)", func() {
			_, _ = KDialogPrompter{Runner: run.ExecRunner{}, Timeout: brief}.Prompt(ctx, "id_test")
		}},
		{"graphical passphrase prompt (zenity)", func() {
			_, _ = ZenityPrompter{Runner: run.ExecRunner{}, Timeout: brief}.Prompt(ctx, "id_test")
		}},
