...
// bitwardenBin is the Bitwarden CLI. Unlike SecretService (PAM-linked
// wallet unlock) and OnePassword (the 1Password desktop app's own
...
// official bw feature — so Bitwarden prompts for the master password
...
// bitwardenItemRef is the subset of `bw get item`'s JSON output Bitwarden
...
// Bitwarden stores and retrieves passphrases as Login items in a
// Bitwarden (or self-hosted Vaultwarden) account via the bw CLI, the same
// shell-out pattern SecretTool and OnePassword use.
//
...
type Bitwarden struct {
	...
	// batching several calls under one Unlock/Lock (see Session).
	Session string
	// held is true between an external Unlock and its matching Lock (see
	// Session): Lookup/Store/Delete/List skip their own prompt/unlock/
	...
	// selects run.DefaultInteractiveTimeout.
...
func (b *Bitwarden) run(ctx context.Context, c run.Cmd) (run.Result, error) {
	if c.Timeout <= 0 {
		c.Timeout = b.Timeout
	}
	if c.Timeout <= 0 {
...
func (b *Bitwarden) Unlock(ctx context.Context) error {
	...
	check, err := b.run(ctx, run.Cmd{Name: bitwardenBin, Args: []string{"login", "--check"}})
	if err != nil {
		return err
	}
	if check.Code != 0 {
...
func (b *Bitwarden) Lock(ctx context.Context) error {
	res, err := b.run(ctx, run.Cmd{Name: bitwardenBin, Args: []string{"lock"}, Env: b.env()})
	b.Session = ""
	b.held = false
	if err != nil {
		return err
...
// caller (see Session). A non-zero exit is treated as a miss, not an
// error — bw does not distinguish "item not found" from other failures by
// exit code alone, the same ambiguity SecretTool accepts.
func (b *Bitwarden) Lookup(ctx context.Context, service string) (string, bool, error) {
	if !b.held {
...
// Session).
func (b *Bitwarden) Store(ctx context.Context, service, label, passphrase string) error {
	if !b.held {
		if err := b.Unlock(ctx); err != nil {
			return err
		}
...
// success, not an error — the same contract as SecretService.Delete,
...
// open by a batching caller (see Session).
func (b *Bitwarden) Delete(ctx context.Context, service string) error {
	if !b.held {
		if err := b.Unlock(ctx); err != nil {
			return err
		}
...
// already held open by a batching caller (see Session).
func (b *Bitwarden) List(ctx context.Context) ([]string, error) {
	if !b.held {
		if err := b.Unlock(ctx); err != nil {
			return nil, err
		}
	...
	res, err := b.run(ctx, run.Cmd{Name: bitwardenBin, Args: []string{"list", "items"}, Env: b.env()})
	if err != nil {
		return nil, err
	}
	if res.Code != 0 {
