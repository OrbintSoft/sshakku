...
// SecretService when the machinery underneath it disappears partway
...
func establishLiveSession(t *testing.T) *SecretService {
	t.Helper()
	...
	backend := &SecretService{Client: client, User: midSessionUser}
...
// caller forever.
func lookupWithinTimeout(t *testing.T, backend *SecretService) error {
	t.Helper()
	type result struct {
		found bool
		err   error
