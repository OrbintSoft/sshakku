...
// API Keychain needs; the darwin build provides the real
...
// Keychain keeps passphrases in the macOS keychain as generic-password
// items, one per service, all under the same account. Unlike
// SecretTool/OnePassword it never shells out to a child
...
func bounded[T any](b *Keychain, call func() (T, error)) (T, error) {
	timeout := b.Timeout
	if timeout <= 0 {
...
func boundedErr(b *Keychain, call func() error) error {
	_, err := bounded(b, func() (struct{}, error) { return struct{}{}, call() })
	return err
}

// find reads the item for service, bounded.
func (b *Keychain) find(service string) (keychainSecret, error) {
...
func (b *Keychain) Lookup(ctx context.Context, service string) (string, bool, error) {
...
// in place if one already exists. Unlike OnePassword (whose CLI has
...
// another's.
func (b *Keychain) Store(ctx context.Context, service, label, passphrase string) error {
...
func (b *Keychain) Delete(ctx context.Context, service string) error {
...
func (b *Keychain) List(ctx context.Context) ([]string, error) {
