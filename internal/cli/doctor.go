...
// binary under their credentials (crossuser.Exec), never by guessing.
//
...
// wallet.Backend, independent of which real backend backend.Open
...
// per-login token from their own kernel keyring (crossuser.Exec), rather than
