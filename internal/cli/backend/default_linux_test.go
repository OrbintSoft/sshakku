...
// secretservice.NewClient fails and Open hands back a
...
func TestNewSecretBackendFallback(t *testing.T) {
	...
	backend, closeFn := Open(t.Context(), "alice", fakeLogger{}, config.Settings{SecretBackend: config.SecretBackendSecretService})
	defer closeFn()
