...
// Open opens the secret backend settings.SecretBackend selects
...
func Open(ctx context.Context, user string, log keys.Logger, settings config.Settings) (wallet.Backend, func()) {
...
func (p walletPasswordPrompter) Prompt(ctx context.Context, keyname string) (string, error) {
	...
	//coverage:ignore
	return dialog.TTY{}.Prompt("Enter "+keyname, true)
}
...
func newWalletPasswordPrompter(ctx context.Context, settings config.Settings, log keys.Logger) prompt.Prompter {
	return walletPasswordPrompter{graphical: dialog.Graphical(ctx, settings, log)}
}
