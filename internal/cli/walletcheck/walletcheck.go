// Package walletcheck describes the configured wallet as this machine actually
// is: which one would be opened, how it would be reached, and what of that is
// present here.
//
// `sshakku doctor` reports it, and `--fix` acts on it, so what it finds is what
// decides whether anything gets created. It only looks; MakeCompartment is the
// one thing here that writes, and only --fix ever calls it.
package walletcheck
...
// View describes the configured wallet as this machine actually is.
func View(ctx context.Context, settings config.Settings) diagnose.WalletView {
	return walletView(ctx, settings, realProbe())
}

// realProbe looks at the machine this is running on.
func realProbe() walletProbe {
	return walletProbe{
...
func (p walletProbe) keepassxcView(ctx context.Context, settings config.Settings) diagnose.WalletView {
	...
	if route == "" || route == config.KeePassXCRouteAuto {
		route = backend.KeePassXCRouteFor(p.goos)
	}
