...
func compartmentView(look walletcheck.SecretServiceLook, hasScreen bool) diagnose.WalletView {
	return diagnose.WalletView{
		...
		Requirements: []diagnose.Requirement{walletcheck.CompartmentRequirement(theCompartment, look, hasScreen)},
	}
}
...
func TestDoctorMakesTheCompartment(t *testing.T) {
	answering := walletcheck.SecretServiceLook{Running: true}
	...
	t.Run("--fix makes it and says what it made", func(t *testing.T) {
		wallet := &walletSpy{views: []diagnose.WalletView{
			...
			compartmentView(walletcheck.SecretServiceLook{Running: true, CollectionFound: true}, true),
		}}
	...
	t.Run("a compartment nobody could ask about is neither made nor pronounced on", func(t *testing.T) {
		...
		wallet := &walletSpy{views: []diagnose.WalletView{compartmentView(walletcheck.SecretServiceLook{}, true)}}
		maker := &makerSpy{made: theCompartment}
		d := compartmentDeps(t, wallet, maker)
