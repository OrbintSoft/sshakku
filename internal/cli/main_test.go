...
// package tests.
// posixDialect is the Bourne form every POSIX shell reads — what a caller that
// named no shell gets, and what these tests compare printed lines against.
func posixDialect(t *testing.T) shell.Dialect {
	t.Helper()
	d, err := shell.FromArgs(nil)
	require.NoError(t, err, "the default shell form must resolve")
	return d
}
...
func TestRun(t *testing.T) {
	tests := []struct {
		...
		{"internal read socket token", []string{crossuser.ReadSocketTokenCmd}, 0},
...
// actually calls. Everything below it is exercised through dispatch with
// substituted seams; this asks the one question that leaves: that the arguments
// a person typed reach a command at all, wired to the real implementations.
//
// `help` is the one command that reaches outside nothing — it neither opens a
// wallet nor starts an agent — so running it here proves the wiring without
// touching the machine the test runs on.
func TestMainDispatchesWhatTheProcessWasGiven(t *testing.T) {
	var out, errOut bytes.Buffer

	code := Main(t.Context(), &out, &errOut, "sshakku", []string{"help"})

	assert.Zero(t, code, "asking for help is not a failure; stderr=%q", errOut.String())
	assert.Contains(t, out.String(), "sshakku <command>",
		"and what the process prints is the command's own output, not something the entry point made up")
}
