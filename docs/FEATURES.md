...
| F34 | Which files SSHakku treats as your keys is yours to state: the directory it looks in, and the names it recognises there. Say so and a key called `work-github` is loaded exactly like `id_rsa`, and `sshakku doctor` reports the directory it really read rather than the one it would otherwise have assumed. A directory you named that is not there is a mistake and is reported as one — an account that simply has no `~/.ssh` is still no error. What no configuration can do is make SSHakku offer something that is not a key of yours: public keys, and the files OpenSSH keeps for itself in that directory, are never loaded however wide the names you allow. | A passphrase-protected key called `work-github` in `~/.ssh` is ignored at login and absent from `sshakku doctor`. Add `key_patterns = ["id_*", "work-*"]` and open a shell: it is asked for once, loaded, and listed. Point `key_dir` at a directory of your own and the keys loaded and reported are the ones in it, under its name; widen the patterns to `["*"]` there and `known_hosts`, `config` and the `.pub` files are still not treated as keys; point `key_dir` at a directory that does not exist and the session log says so. |
