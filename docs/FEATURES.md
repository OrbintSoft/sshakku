...
| F28 | Upgrading SSHakku never asks you for a passphrase you have already stored. When a release changes how an entry is named or where it is kept, what is already in your wallet keeps working and is moved across for you — you are not asked to retype it, and nothing is left behind that `sshakku forget` can no longer reach. | Store a passphrase, upgrade, and open a new shell: the key loads silently, exactly as before, and `sshakku forget --all` still removes it afterwards. |
