...
| A graphical passphrase prompt is raised where one can be (F4) | ✅ `TestKDialogPrompt`, `TestKDialogAvailable`, `gui_linux_test.go` — a display server that answers plus kdialog installed | ❌ macOS has no graphical prompter implemented, so every passphrase is asked for on the terminal. `guiprompt_darwin.go` states it rather than leaving it to be inferred from kdialog being absent. Not an absence: a Mac in a login session always has a window server, and `osascript … with hidden answer` would draw the prompt. See PLAN.md Phase 17 |
