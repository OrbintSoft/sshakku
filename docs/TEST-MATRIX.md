...
| A graphical passphrase prompt is raised where one can be (F4) | ✅ `TestKDialogPrompt`, `TestKDialogAvailable`, `gui_linux_test.go` — a display server that answers plus kdialog installed; `TestGraphicalPrompterWithASessionAndKDialog` and `TestNoGraphicalPrompterWithNoDisplayServer` pin both answers of the wiring itself | ❌ macOS has no graphical prompter implemented, so every passphrase is asked for on the terminal. `guiprompt_darwin.go` states it rather than leaving it to be inferred from kdialog being absent. Not an absence: `osascript … with hidden answer` would draw the prompt with nothing extra installed. Note for whoever implements it that "on a Mac" is not the condition — a single-user boot and an SSH login are both sessions with no window server, so the session's own kind has to be probed. See PLAN.md Phase 17 |
...
| An upgrade never asks for a passphrase already stored (F28) | ⚠️ `test/bats/shell-plumbing.bats` seeds the old entry name and loads with no terminal, but it needs the kernel user keyring for the askpass handoff and the container tier has no PAM login session, so it **skips** there rather than running. What actually holds this up on Linux is `TestLookupWithLegacy*` — a miss under the current name, a hit under the old one, and the move that follows — plus the macOS run of the same shell case | ✅ `test/bats/shell-plumbing.bats` — the entry is seeded under the old name only, `load-keys` runs with no terminal to fall back to, and the key reaches the agent; then the entry is checked to have moved to the current name and gone from the old |
