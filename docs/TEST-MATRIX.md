...
| A key gone from the agent is refilled from the wallet, in a shell already open, with no terminal and no graphical prompter (F6) | ✅ `test/bats/askpass-broker.bats` — the installed login hook is sourced, then a real `ssh-add` runs under `setsid` (no controlling terminal, so nothing can fall back to a prompt) with no handoff token, so only the wallet can answer; the session log line naming the wallet is asserted, not merely the key reaching the agent | ❌ (the bats suite does not run on macOS yet) |
| The askpass broker is wired in a session with no graphical prompter (F6) | ✅ `test/bats/askpass-broker.bats` and `TestAskpassEnvHeadless` — with no `DISPLAY`/`WAYLAND_DISPLAY` and no `kdialog`, `askpass-env` still emits all three exports | ✅ `TestAskpassEnvHeadless` |
| A wallet that never answers — present but locked behind an unlock prompt nobody can answer — does not hold up an `ssh` or a login shell (F21) | ✅ `test/bats/askpass-broker.bats` (both entry points, against a `secret-tool` that never returns) and `TestNoCommandBlocksIndefinitely` (every external program this code runs) | ✅ `TestNoCommandBlocksIndefinitely` |
