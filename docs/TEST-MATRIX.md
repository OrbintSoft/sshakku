...
| A graphical passphrase prompt is raised where one can be, and only there (F4, F29) | ✅ `TestKDialogPrompt`, `TestKDialogAvailable`, `gui_linux_test.go` — a display server that answers plus kdialog installed; `TestGraphicalPrompterWithASessionAndKDialog` and `TestNoGraphicalPrompterWithNoDisplayServer` pin both answers of the wiring itself | ⚠️ `TestGraphicalSession` and `guiprompt_darwin_test.go` cover the decision — Aqua gets the dialog, an SSH login and a single-user boot get the terminal — with a `launchctl` on PATH answering as the real one does. The hosted runner's own session reports Aqua, so `test/bats/shell-plumbing.bats` does raise a real dialog with nobody in front of it, and shows sshakku giving up on it inside its budget (F21). What no job can show is the window being usable: nobody is at that screen to read or answer it. That half waits on a run on a real Mac |
