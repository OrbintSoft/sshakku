...
| `internal/keys/wallet` | Stores and retrieves a key's passphrase: the pluggable backends (Secret Service, `secret-tool`, the macOS Keychain, KeePassXC over its local protocol or its CLI, 1Password, Bitwarden) behind one `wallet.Backend`, plus how SSHakku names its own entries in a store it shares with everything else. |
