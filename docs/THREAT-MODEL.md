...
| I11 | **GNOME Keyring's own at-rest encryption is trust-model-dependent, unlike KDE's.** GNOME Keyring rejects a custom Secret Service alias outright (confirmed live against `gnome-keyring-daemon` 48), so `internal/secretservice.Client` auto-provisions sshakku's dedicated collection unaliased, found again by its `Label` (open decision 7). The only way that collection unlocks non-interactively on every later shell (rather than a fresh GUI dialog each time — `gnome-keyring-daemon`'s only automatic-unlock path is a **blank password**, confirmed by reading `gkd-secret-unlock.c`) is if the user answers the one-time creation dialog with a blank password — which GNOME itself stores **unencrypted on disk** (`~/.local/share/keyrings/*.keyring`, readable by anyone with file access, not just same-UID processes). KDE's `ksecretd` backend does not force this trade-off. | Present (open) | The choice is the user's at collection-creation time: a blank password (silent every shell, plaintext at rest) or a real one (encrypted at rest, an interactive prompt every new shell — no PAM-hash auto-unlock exists for a non-default GNOME collection). **Deferred**: sshakku does not yet warn about this trade-off at creation time (e.g. via `doctor`); tracked here rather than assumed obvious. |
