...
# GOPOGH_COMMIT, GO_BETTER_HTML_COVERAGE_COMMIT, and GO_IGNORE_COV_COMMIT
# already set in the environment (the workflow's job-level env: block provides
# these).
set -euxo pipefail
...
go install "github.com/quantumcycle/go-ignore-cov@${GO_IGNORE_COV_COMMIT}"                # v${GO_IGNORE_COV_VERSION}
