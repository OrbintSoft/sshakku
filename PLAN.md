...
| Windows batch (`cmd/*/testdata/*.cmd`) | **No linter.** Nothing exists for batch that is worth adding — there is no shellcheck for it, and the candidates are style checkers for a language whose hazards are parsing ones. It is checked two other ways instead: `editorconfig-checker` covers charset, final newline and line endings, and the file is a test fixture, so the six `config --edit` tests fail the moment it stops behaving. It is also the one file type `.gitattributes` and `.editorconfig` check out as CRLF — cmd.exe seeks through a script to find a label, and does not reliably find one when the lines end in LF alone |
