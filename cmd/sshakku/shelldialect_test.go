...
func dialect(t *testing.T, name string) shellDialect {
	t.Helper()
	d, err := shellDialectNamed(name)
...
func TestEachDialectPrintsWhatItsShellReads(t *testing.T) {
	posix, powershell := dialect(t, shellPosix), dialect(t, shellPowerShell)
...
func TestTheDialectIsTheOneAskedForOrPosix(t *testing.T) {
	answered := []struct {
		...
		{"no flag", nil, shellPosix},
		{"posix by name", []string{"--shell=posix"}, shellPosix},
		{"posix as a separate word", []string{"--shell", "posix"}, shellPosix},
		{"powershell by name", []string{"--shell=powershell"}, shellPowerShell},
		{"powershell as a separate word", []string{"--shell", "powershell"}, shellPowerShell},
	}
	for _, tc := range answered {
		t.Run(tc.name, func(t *testing.T) {
			got, err := dialectFromArgs(tc.args)
	...
	assert.Contains(t, err.Error(), shellPowerShell, "and what could have been asked for instead")
}
