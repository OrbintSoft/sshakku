...
// wallet.SecretService, which unlocks its own dedicated collection only for the
...
// D-Bus user session) it logs the failure and falls back to wallet.SecretTool,
...
func newDefaultSecretBackend(ctx context.Context, user string, log keys.Logger, settings config.Settings) (wallet.Backend, func()) {
	client, err := secretservice.NewClient(ctx)
	if err != nil {
		_ = log.Log("ERROR", fmt.Sprintf("secret service: %v; falling back to secret-tool", err))
		return wallet.SecretTool{Runner: run.ExecRunner{}, User: user, Timeout: settings.CommandTimeout}, func() {}
	}
